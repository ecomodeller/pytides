...
class Tide(object):
    ...
    def __init__(
        ...
        if np.all([constituents, amplitudes, phases]):
            if len(constituents) == len(amplitudes) == len(phases):
                ...
                raise ValueError(
                    "Constituents, amplitudes and phases should \
				 all be arrays of equal length."
                )
        elif model is not None:
            if not model.dtype == Tide.dtype:
                raise ValueError(
                    "Model must be a numpy array with \
				dtype == Tide.dtype"
                )
        else:
            raise ValueError(
                "Must be initialised with constituents, \
    ...
    def form_number(self):
        ...
        tides.
        """
        MainC = [constituent._K1, constituent._O1, constituent._M2, constituent._S2]
        k1, o1, m2, s2 = (
